...
Baseline measured on 2026-07-13:
...
- Group accepted events by source, target, ability, kind, and message shape.
...
For every fight-boundary change, verify:

- A slain mob ends the fight.
- Late damage involving that same mob remains in the completed fight during the
  eight-second death grace period.
- Damage involving a different mob starts a new fight immediately.
- The local player's death ends the fight immediately.
- A combat pause ends the fight after the configured idle timeout.
- Replay uses log timestamps, while live idle detection uses wall-clock time.

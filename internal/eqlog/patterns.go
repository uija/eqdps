...
var eventPatterns = []eventPattern{
	...
	// optional parenthesized amount, optional full spell name (including rank), annotation.
	{data.LogRowEventTypeHealing, regexp.MustCompile(`^(.+?) healed (.+?)(?: (over time))? for ([0-9]+)(?: \(([0-9]+)\))? hit points?(?: by (.+?))?\.(?: \(([^)]+)\))?$`), []string{" healed ", " for ", " hit point"}},
	...
	{data.LogRowEventTypeFailedMeleeOthers, regexp.MustCompile(`^(.+?) tries to (backstab|bash|bite|cleave|claw|crush|frenzy on|hit|kick|maul|pierce|punch|reave|shoot|slash|slice|smash|smite|sting|strike) (.+?), but (misses|.+? (?:block|blocks|dodge|dodges|parry|parries|riposte|ripostes)|.+? magical skin absorbs the blow)!(?: \(([^)]+)\))?$`), []string{" tries to ", ", but "}},
	...
	{data.LogRowEventTypeExperience, regexp.MustCompile(`^You gain (?:party )?experience(?: \(with a bonus\))?! \(([0-9]+(?:\.[0-9]+)?)%\)$`), []string{"You gain ", "experience", "%"}},
	{data.LogRowEventTypeKillExperienceReward, regexp.MustCompile(`^You gain (?:party )?experience(?: \(with a bonus\))?!(?: \([0-9]+(?:\.[0-9]+)?%\))?$`), []string{"You gain ", "experience"}},
	...
	{data.LogRowEventTypeParcelSent, regexp.MustCompile(`^(.+?) told you, 'I will deliver the (?:(?:stack of )?([0-9]+) )?(.+) to ([^']+) as soon as possible!'$`), []string{" told you, 'I will deliver the ", " as soon as possible!'"}},
	{data.LogRowEventTypeParcelReceived, regexp.MustCompile(`^You have received a new parcel delivery containing (?:(?:a stack of )?([0-9]+) )?(.+) from (.+)!$`), []string{"You have received a new parcel delivery containing ", " from ", "!"}},
	{data.LogRowEventTypeParcelCollected, regexp.MustCompile(`^(.+?) hands you the (?:(?:stack of )?([0-9]+) )?(.+) that was sent from (.+)\.$`), []string{" hands you the ", " that was sent from ", "."}},

...
func Detect(logFile string, spells []Spell) (Source, []string, bool) {
...
// Available reports whether every icon referenced by spells has already been
// extracted for setName.
func Available(targetDir, setName string, spells []Spell) bool {
	if setName == "" || len(spells) == 0 {
		return false
	}
	seen := make(map[int]bool)
	for _, spell := range spells {
		if spell.IconID < 0 || seen[spell.IconID] {
			continue
		}
		seen[spell.IconID] = true
		if !isRegularFile(SetIconPath(targetDir, setName, spell.IconID)) {
			return false
		}
	}
	return len(seen) > 0
...
func Extract(source Source, targetDir string, spells []Spell) error {
	sets, err := discoverIconSets(source, spells)
	if err != nil {
		return err
	}
...
func ExtractAll(source Source, targetDir string, spells []Spell) ([]string, error) {
	sets, err := discoverIconSets(source, spells)
	if err != nil {
		return nil, err
	}
	names := make([]string, 0, len(sets))
	for _, set := range sets {
		if !Available(targetDir, set.name, spells) {
...
func extractSet(set iconSet, targetDir string, spells []Spell) error {
...
func discoverIconSets(source Source, spells []Spell) ([]iconSet, error) {
...
func requiredSheets(spells []Spell) []int {
	seen := make(map[int]bool)
	for _, spell := range spells {
